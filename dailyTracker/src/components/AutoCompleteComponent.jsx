...
const AutoCompleteComponent = ({optionsList, label="Select an option", handleFoodSelection}) => {
  return (
    <Card
        variant="outlined"
        sx={{
          ...
          <h2 className="font-semibold pb-2 text-white text-left">{label}</h2>
           <Autocomplete
            ...
            PaperComponent={({ children }) => (
              <Paper style={{ background: '#0f172a', color: 'white' }}>{children}</Paper>
            )}
            renderInput={(params) => <TextField {...params} label="Select a food" />}
          />
        </CardContent>
      </Card>
  )
